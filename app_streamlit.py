...
password_ok = True  # Par défaut True pour les rôles sans mot de passe
...
if password_ok:
    # --- Étape 2 : Filtres spécifiques selon rôle ---
    ...
    else:  # Étudiant / Professeur
        menu_options = ["Mon planning"]

    menu = st.sidebar.selectbox("Menu", menu_options)

    # --- Étape 4 : Fonctions pour filtrer les données ---
    def filter_examens_by_dept(df, dept_id):
        df = df.merge(backend.get_formations(), left_on="formation_id", right_on="id")
        return df[df["dept_id"] == dept_id]

    def filter_examens_by_user(df, user_id):
        if user_id in backend.get_etudiants_ids():
            inscriptions = backend.get_inscriptions()
    ...
    if menu in ["Emplois du temps", "Emplois du temps département", "Mon planning", "Vue globale"]:
        ...
        df_conflits = pd.DataFrame(backend.get_conflits_etudiants())

        if menu == "Conflits par formation":
            df_conflits = df_conflits.merge(backend.get_etudiants(), left_on="etudiant_id", right_on="id")
